...
class Managers extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model('managers/event');
	...
	public function index()
	{
		...
		$data['details'] = $this->event->get('1');
		$data['status'] = $this->event->status();
		//list of all the teamids who are in a particular event
		$eventids =  $this->event->get($this->session->userdata('user_id'));
		//if($eventids === FALSE)
       ...
       if($eventids !=FALSE)
		  foreach ($eventids as $index => $eventid) 
		  {
			...
			$teamDetails = $this->event->get_details($teamid, 'array');
			...
			for ($i=1; $i<=$teamDetails["tsize"]; $i++) {
				...
				array_push($teams[$teamid]["users"], $this->event->get_user_details($teamDetails["user".$i."id"]));
			}
		}
	...
	public function changestatus($teamid = '')
	{
		...
		if($this->form_validation->run() === TRUE){
			if($this->event->changestatus($teamid,$this->input->post('changestatus') ,$this->session->userdata('user_id')) === TRUE)
				{echo "1"; return;}
			else
				{echo "0"; return;}
		}
